...
The load balancer provides two versions of its internal engine for processing traffic: `v1` and `v2`. Some behaviors, such as HTTP traffic handling, may differ depending on the engine version.

| Engine Version | Description |
| -- | -- |
| v2 | The latest engine version. It is applied by default to newly created load balancers, and you can use features that are only available in the latest engine, such as HTTP/2. |
...
!!! danger "Caution"
    ...
    * HTTP response chunk processing: `v2` can merge HTTP responses that are sent in multiple chunks into a single response. Clients that rely on receiving responses in individual chunks may behave differently.
    ...
    * HTTP standard compliance: `v2` adheres more strictly to the HTTP standard. If you were using requests or responses in a non-standard format, behavior may change.

    `v2` complies with the HTTP standard (RFC), but some behaviors may differ slightly from the previous version (`v1`) as a result. The items listed above are representative examples, and other behaviors not listed here may also change. After changing the engine version, make sure to perform sufficient validation before applying it to a production environment.
...
If you select HTTP/2, communication uses H2C (plaintext) when HTTP is selected in the member group, or H2 (TLS encryption) when HTTP_REENCRYPT is selected.
The load balancer operates strictly according to the selected protocol version, and if HTTP/2 is selected, communication over HTTP/1 is not possible.
