...
!!! danger "Caution"
    Changing the engine version may alter how HTTP traffic is processed as follows. Make sure to test before applying to a production environment.

    * HTTP response chunk processing: `v2` can merge HTTP responses that are split into multiple chunks into a single response. Clients that assume responses arrive in individual chunks may behave differently.
    * HTTP header name formatting: `v2` may convert HTTP/1.1 header names in requests and responses to lowercase before forwarding them (e.g., `Content-Type` → `content-type`). Although HTTP header names are case-insensitive by standard, backend servers or clients that treat header names as case-sensitive may be affected. In particular, clients that read response headers may be impacted.
    * HTTP standard compliance: `v2` enforces HTTP standards more strictly. If you have been using requests or responses in a non-standard format, behavior may change.
...
If you select HTTP/2, the load balancer communicates using H2C (plaintext) when HTTP is selected for the member group, or H2 (TLS encryption) when HTTP_REENCRYPT is selected.
