...
The load balancer provides two versions of the internal engine that processes traffic: `v1` and `v2`. Depending on the engine version, some behavior may differ, such as HTTP traffic handling.
...
!!! danger "Caution"
    Changing the engine version may alter how HTTP traffic is processed, as described below. Make sure to test thoroughly before applying changes to your production environment.

    * HTTP response chunk processing: `v2` can merge multiple HTTP response chunks sent in parts into a single response. Clients that rely on receiving responses in individual chunks may behave differently.
    ...
    * HTTP standards compliance: `v2` adheres more strictly to HTTP standards. If you are using requests or responses in non-standard formats, behavior may change.

    `v2` complies with HTTP standards (RFC), but in doing so, some behaviors may differ slightly from those of the previous version (`v1`). The items listed above are representative examples, and other behaviors not explicitly mentioned may also change. After changing the engine version, make sure to perform sufficient validation before applying the changes to your production environment.
...
If you select HTTP or HTTPS as the health check protocol, it operates the same as the protocol version selected in the member group.
