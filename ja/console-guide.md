...
4. 登録された証明書リストから次の情報を確認できます。
   - **名前**: 証明書名
   - **有効期限**: 証明書有効期限および有効期限まで残り日数

##### 証明書追加
1. リスナー詳細画面の**証明書**タブで**+ 証明書追加**ボタンをクリックします。
2. Certificate Manager使用の有無を選択します。
...
> [注意] 証明書追加時にロードバランサーが再起動されます。再起動する過程で、既存の接続されたセッションは維持されますが、新しいセッションは処理できません(約1秒未満)。したがって、サービスに影響を与えない時間に進行することを推奨します。

##### 基本証明書変更
1. リスナー詳細画面の**証明書**タブで**基本証明書変更**ボタンをクリックします。
2. 基本証明書として使用する証明書を選択します。
3. 警告メッセージを確認した後、チェックボックスを選択して**確認**をクリックします。

> [注意] 基本証明書変更時にロードバランサーが再起動されます。再起動する過程で、既存の接続されたセッションは維持されますが、新しいセッションは処理できません(約1秒未満)。したがって、サービスに影響を与えない時間に進行することを推奨します。
...
> [注意] 基本証明書は削除できません。基本証明書を削除するには、まず他の証明書を基本証明書に変更した後、削除する必要があります。

> [注意] 証明書削除時にロードバランサーが再起動されます。再起動する過程で、既存の接続されたセッションは維持されますが、新しいセッションは処理できません(約1秒未満)。したがって、サービスに影響を与えない時間に進行することを推奨します。

### カスタムレスポンスガイド

ロードバランサーのリスナーでカスタムレスポンスを設定できます。カスタムレスポンスを活用すると、特定のHTTPエラーコードが発生した時に、希望するカスタムメッセージやHTMLなどの内容を直接ユーザーに伝達できます。

#### カスタムレスポンス確認および設定

1. ロードバランサー詳細画面で**リスナー**タブをクリックします。
2. カスタムレスポンスを設定するリスナーを選択します。
3. リスナー詳細画面で**カスタムレスポンス設定確認/変更**をクリックします。
4. 次の項目を入力および確認できます。
   - **レスポンスコード**: カスタムレスポンスを適用するHTTPステータスコードを選択します。（サポートコード: 400、403、408、500、502、503、504）
   - **コンテンツタイプ**: ユーザーに伝達するレスポンスのContent-Typeを選択します。（`text/html`、`text/plain`、`application/json`、`application/javascript`、`text/css`から選択）
   - **レスポンス本文**: ユーザーに表示するレスポンスの本文を入力します。（最大1024文字、入力内容はコンテンツタイプに応じてHTML、テキストなど自由に記述）
5. 各項目を入力後、**確認**をクリックするとレスポンスが作成されます。作成されたレスポンスはリストで確認できます。

#### カスタムレスポンス削除

- 作成したカスタムレスポンスはリストから希望する項目を選択して**削除**ボタンをクリックして削除できます。
- 削除されたレスポンスに該当するエラーコード発生時、基本システムレスポンスが表示されます。

> [参考] 同じリスナー内で各エラーコードは一度だけカスタムレスポンスとして登録できます。

> [注意] カスタムレスポンスを追加、修正、または削除する時にリスナーが一時的に（1秒未満）再起動される可能性があるため、サービス影響が少ない時間に変更作業を進行することを推奨します。

### X-Forwardedヘッダー設定ガイド

ロードバランサーのリスナーでX-Forwardedヘッダー設定を確認および変更できます。X-Forwardedヘッダーはクライアントの元の情報（プロトコル、ポート、IPアドレス）をバックエンドサーバーに伝達するために使用されます。

#### X-Forwardedヘッダー確認および設定

1. ロードバランサー詳細画面で**リスナー**タブをクリックします。
2. X-Forwardedヘッダーを設定するリスナーを選択します。
3. リスナー詳細画面の**基本情報**タブで**X-Forwardedヘッダー**項目の**設定確認/変更**ボタンをクリックします。
4. 次の項目を設定できます。
   - **X-Forwarded-Proto**: クライアントが使用したプロトコル（httpまたはhttps）をバックエンドサーバーに伝達するかどうかを設定します。**使用**または**使用しない**のいずれかを選択します。
   - **X-Forwarded-Port**: クライアントが接続したポート番号をバックエンドサーバーに伝達するかどうかを設定します。**使用**または**使用しない**のいずれかを選択します。
   - **X-Forwarded-For**: クライアントの元のIPアドレスをバックエンドサーバーに伝達するかどうかを設定します。**使用**または**使用しない**のいずれかを選択します。
5. 各項目を設定した後、警告メッセージのチェックボックスを選択して**確認**をクリックすると設定が適用されます。

> [注意] X-Forwardedヘッダー設定を変更するとロードバランサーが再起動されます。再起動する過程で、既存の接続されたセッションは維持されますが、新しいセッションは処理できません(約1秒未満)。したがって、サービスに影響を与えない時間に進行することを推奨します。
