...
##### Cert Manager使用
リスナーでTERMINATED_HTTPSプロトコルを使用する場合の認証書の登録方法は、CertManagerに登録した認証書を使用する方法と直接登録する方法の2つです。

*Cert Managerサービスに認証書を登録し、リスナーにその認証書を接続すると、電子メールで認証書満了日のアラームを受けることができます。
*リスナーに直接認証書を登録した場合は、認証書満了日の通知がありません。 ただし、コンソールのリスナー画面で満了日を確認することはできます。
> [注意事項]
> Cert Manager サービスで認証書を更新した場合、影響を受けるリスナーの認証書も一緒に更新しなければなりません。
> Cert Managerに登録した認証書をリスナーに使用するには、"プライベートキー"のパスワードが必ず削除されなければならず、PKCS#1またはPKCS#8 PEM形式でなければなりません。
