...
### エンジンバージョンごとのサポート機能 { #features-supported-by-engine-version }
...
!!! danger "注意"
    エンジンバージョンを変更すると、以下のように HTTP トラフィックの処理動作が変わる場合があります。本番環境に適用する前に、必ずテストを実施してください。

    * HTTP レスポンスのチャンク処理: `v2` は複数のチャンクに分割されて送信される HTTP レスポンスを 1 つに結合して処理できます。レスポンスがチャンク単位で到着することを前提に動作するクライアントの場合、動作が変わる可能性があります。
    ...
    * HTTP 標準への準拠: `v2` は HTTP 標準をより厳密に準拠します。非標準形式のリクエスト/レスポンスを使用していた場合、動作が変わる可能性があります。
    ...
    `v2` は HTTP 標準（RFC）に準拠していますが、その過程で既存（`v1`）と比較して一部の動作が変わる場合があります。上記の項目は代表的な例であり、明示されていない他の動作も変わる可能性があるため、エンジンバージョンを変更した後は、必ず十分に検証したうえで本番環境に適用してください。
