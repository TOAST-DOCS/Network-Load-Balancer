...
ロードバランサーは[VPC](/Network/VPC/ko/overview/#_2)の[サブネット](/Network/VPC/ko/overview/#_2)に作成されます。ロードバランサーは作成時に指定されたサブネットからIPを割り当てられ、自分のIPで使用します。また指定されたサブネットと同じVPCに属するインスタンスをメンバーに登録し、流入したトラフィックを分配します。該当VPCとPeering接続した他のVPCに属すインスタンスもメンバーに追加できます。それ以外のインスタンスは追加できません。
...
ロードバランサーが処理したトラフィック監視の統計指標を、チャートで確認できます。 TOASTロードバランサー統計機能の特徴は次のとおりです。
