...
!!! danger "注意"
    ...
    * HTTP レスポンスチャンク処理: `v2` は複数のチャンクに分割されて送信される HTTP レスポンスを1つに結合して処理できます。レスポンスがチャンク単位で到着することを前提に動作するクライアントの場合、動作が変わる場合があります。
...
ヘルスチェックプロトコルを HTTP、HTTPS に選択すると、メンバーグループで選択したプロトコルバージョンと同様に動作します。
