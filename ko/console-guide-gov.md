...
#### SSL 정책 생성
SSL 정책을 생성하려면 [SSL 정책 생성] 버튼을 누르고, 다음의 값을 입력합니다.

* 이름: SSL 정책의 이름을 입력합니다.
* 설명: SSL 정책에 대한 설명을 기술합니다.
* 최소 TLS 버전: SSL 정책이 허용하는 최소 TLS 버전을 선택합니다. `SSLv3`, `TLSv1.0`, `TLSv1.0_2016`, `TLSv1.1`, `TLSv1.2`, `TLSv1.3` 중 하나를 선택합니다.
* TLS 1.2 암호화 스위트: TLS 1.2 이하 버전에서 사용할 암호화 스위트를 선택합니다. 최소 TLS 버전이 `TLSv1.3`이면 선택할 수 없습니다.
* TLS 1.3 암호화 스위트: TLS 1.3 연결에서 사용할 암호화 스위트를 선택합니다.

> [주의] 최소 TLS 버전은 생성 후 변경할 수 없습니다. 변경이 필요하면 새 SSL 정책을 생성하여 리스너에 적용해야 합니다.

> [주의] 최소 TLS 버전이 `TLSv1.3`이 아닌 경우, TLS 1.2 암호화 스위트와 TLS 1.3 암호화 스위트 중 최소 하나는 선택해야 합니다.

> [참고] SSL 정책은 프로젝트당 최대 10개까지 생성할 수 있습니다.

<a id='change-ssl-policies'></a>
#### SSL 정책 변경
SSL 정책의 이름, 설명, 암호화 스위트를 변경할 수 있습니다. 최소 TLS 버전은 변경할 수 없습니다.
...
#### SSL 정책 적용
SSL 정책은 리스너 생성 화면 또는 리스너 변경 및 상세 정보 화면에서 리스너에 연결합니다.
