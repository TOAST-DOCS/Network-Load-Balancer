...
> [주의] 최소 SSL/TLS 버전은 생성 후 변경할 수 없습니다. 변경이 필요하면 새 SSL 정책을 생성하여 리스너에 적용해야 합니다.

> [주의] 암호화 스위트는 최소 하나 이상 선택해야 합니다.
...
> [주의] SSL 정책 변경 시 해당 정책이 연결된 리스너의 로드 밸런서가 재기동됩니다. 재기동 과정에서 기존에 연결된 세션은 유지되지만, 새로운 세션은 최대 1초 이하 동안 처리되지 않을 수 있습니다. 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장하며, 변경 화면에서 안내 체크박스를 선택해야 변경이 진행됩니다.
