...
* 작업 우선 순위: L7 규칙 우선순위를 설정합니다. 입력한 값을 기반으로 작업 유형 내 우선순위가 결정되며, 중복된 값을 입력할 경우 새로운 규칙이 우선됩니다. 
  ...
  * 로드 밸런서 생성 이후, **상세 보기**에서 확인할 수 있는 L7 규칙의 작업 우선순위는 사용자가 입력한 우선순위의 절대 값이 아닌 상대 값을 표현합니다. 
...
> [참고] 멤버 그룹이 어느 리스너에도 속해 있지 않다면, 로드 밸런서는 해당 멤버 그룹에 속한 멤버들을 대상으로 상태 확인을 수행하지 않습니다.
