...
* Keepalive Timeout : 로드밸런서가 클라이언트 및 서버와의 세션에 세션 유지 시간을 지정합니다. 로드밸런서 입장에서 상대편이 커넥션을 유지하는 한 이 시간동안 세션유지를 보장합니다. 기본값은 300초로 설정됩니다.
* Proxy Protocol : 서버에서 클라이언트의 IP를 알기 위해 프록시 프로토콜을 사용하도록 설정한 경우, 로드밸런서에도 이 옵션을 활성화하여 프록시 프로토콜을 제공하게 합니다.
