...
* 상태 확인 포트: 상태 확인을 시도할 멤버의 포트를 설정합니다. 0을 입력하면 각 멤버에 설정된 포트 번호로 상태 확인을 수행합니다. 0이 아닌 값을 입력하면 멤버에 설정된 포트와 관련 없이 해당 포트 번호로 상태 확인을 수행합니다.
...
> [주의] 멤버 그룹 내에 IP는 동일하고 포트 번호만 다른 멤버가 다수 존재할 경우, 상태 확인 포트 번호 설정에 유의해야 합니다. 예를 들어, 192.168.0.10의 80 포트, 192.168.0.10의 8080 포트와 같이 2개의 멤버가 존재한다고 가정할 때, 상태확인 포트를 0으로 지정하면 80포트 및 8080 포트 각자를 대상으로 상태확인을 수행합니다. 만약 상태 확인 포트를 80으로 지정하면 멤버 포트가 8080포트라 할 지라도 80포트를 확인합니다. 이 때, 192.168.0.10의 80포트가 활성화 되어있다면 192.168.0.10의 8080포트도 ACTIVE 상태로 간주합니다.
