...
로드밸런서로 유입되는 패킷에 대해 IP 기반 접근제어 기능을 설정할 수 있습니다.

IP 접근제어 기능을 사용하기 위해 설정해야 할 사항은 다음과 같습니다.

* 접근제어 그룹
     * 한 프로젝트에 최대 10개의 그룹을 생성할 수 있습니다.
     * 그룹은 이름, 메모, 접근제어타입 등의 속성을 가집니다.
     * 접근제어 그룹의 속성 중 접근제어 타입은 '허용(Allow)'와 '거부(Deny)'로 구분됩니다.
     * 접근제어 그룹에 제어를 원하는 접근제어 대상 IP 를 추가할 수 있습니다.

* 접근 제어 타입의 의미
     * '허용(Allow)' : 이 그룹에 속한 IP 의 접근을 허용하고, 그 외의 모든 IP의 접근은 차단합니다.
     * '거부(Deny)' : 이 구룹에 속한 IP 의 접근을 차단하고, 그 외의 모든 IP의 접근은 허용합니다.

* 접근제어 대상
     * 한 프로젝트에 최대 1,000개의 엔트리를 생성할 수 있습니다.
     * 하나의 엔트리가 접근제어 대상 IP 하나를 기술합니다.
     * 이름, 메모 속성을 추가 기술할 수 있습니다.

* 접근제어 그룹을 로드밸런서에 적용
     * 잘 관리된 접근제어 그룹을 로드밸런서에 적용할 수 있습니다.
     * 한 접근제어 그룹은 여러 로드밸런서에 적용할 수 있습니다.
     * 한 로드밸런서에 여러 접근제어 그룹을 적용할 수 있습니다. 단, 추가하는 그룹의 접근제어타입이 동일해야 합니다.

> [참고] 시큐리티 그룹과 로드밸런서 IP 접근제어 기능 비교
>
> 시큐리티 그룹와 LB IP 접근제어 기능은 다음과 같은 차이를 보입니다.
>
> | 구분 | 시큐리티 그룹 | LB IP 접근제어 |
> |--|--|--|
> | 제어 대상 | 인스턴스 | 로드밸런서 |
> | 설정 대상 | IP, Port, Protocol 등 설정 | IP 만 설정 |
> | 제어 트래픽 방향 | In / Out 트래픽 방향 설정 가능 | In 트래픽 대상 |
> | 제어 방법| 허용 정책만 설정 가능 | 허용 또는 차단 설정 선택 가능 |
