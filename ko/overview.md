...
로드밸런서가 처리한 네트워크 플로우 관련 여러가지 통계 지표들을 차트를 통해 확인할 수 있습니다. TOAST 로드밸런서 통계 기능의 특징은 다음과 같습니다.
* 로드밸런서별, 리스너별 통계 차트를 제공합니다.
* 1시간, 24시간, 1주, 1달, 지정 기간 등으로 기간을 분류하여 볼 수 있습니다.
* 로드밸런서를 기준으로 클라이언트측의 통계량과 인스턴스측 통계량이 서로 다른 차트로 제공됩니다.
* 인스턴스측 통계량은 멤버 인스턴스별로 구분하여 볼 수 있고, 취합한 결과만을 볼 수도 있습니다. (인스턴스 별로 보기 : ON/OFF)
* 통계 지표는 앞으로 계속 추가될 예정입니다. (통계 확장 기능 : ON/OFF)

제공되는 차트는 다음과 같습니다.

| 통계 지표명 (차트명) | 구분 | 단위 | 설명 | 비고 |
|--|--|--|--|--|
| 클라이언트 세션 개수 | 클라이언트측 | ea | 로드밸런서가 클라이언트와 연결중인 세션수 |  |
| 클라이언트 세션 CPS | 클라이언트측 | cps<br>(connections per second) | 클라이언트와 1초동안 새로 연결한 세션수 | | 
| 세션 CPS | 인스턴스측 | cps<br>(connections per second) | 인스턴스와 1초동안 새로 연결한 세션수 |  |
| 트래픽 In | 인스턴스측 |  bps<br>(bits per second) | 로드밸런서가 인스턴스에게 보낸 트래픽량 | L2,L3,L4 헤더 크기 제외|
| 트래픽 Out | 인스턴스측 | bps<br>(bits per second) | 인스턴스가 로드밸런서에게 보낸 트래픽량 | L2,L3,L4 헤더 크기 제외|
| 로드밸런싱 제외 개수 | 인스턴스측 | ea | 헬스체크 실패로 인해 로드밸런싱 대상에서 제외된 회수| |

> [참고] 제약 사항은 다음과 같습니다.
> * 현재 사용중인 로드밸런서, 리스너, 멤버에 대해서만 통계 차트가 제공됩니다. 로드밸런서 자원을 삭제하는 경우 해당 자원의 과거 통계 데이터는 제공되지 않습니다.
> * 단위가 ea 인 차트에서 설정한 기간에 따라 수치의 의미가 달라질 수 있습니다. 수치의 의미는 개별 차트 상단 물음표에 마우스를 올리면 확인할 수 있습니다.
> * 트래픽 In, 트래픽 Out 등 네트워크 사용량 관련된 지표에서 표현되는 데이터는 L2, L3 L4 헤더의 크기가 제외된 페이로드의 전송 크기를 단위시간으로 나눈 데이터입니다. 따라서 차트에 표현되는 수치는 과금 데이터와 무관합니다.
 ...
 * 로드밸런서 사용 요금: **ACTIVE** 상태인 로드밸런서가 사용된 기간만큼 요금이 청구됩니다. 이용 요금에 대한 자세한 사항은 [로드밸런서 타입별 요금](/service/network/load_balancer)을 참고하시기 바랍니다.
