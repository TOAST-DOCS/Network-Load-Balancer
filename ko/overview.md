...
> [주의] 최소 TLS 버전이 `TLSv1.3`인 경우 `ciphers`에 TLS 1.2 이하 암호화 스위트를 포함할 수 없습니다. TLS 1.3 정책에서는 TLS 1.2 핸드셰이크가 일어나지 않아 TLS 1.2 암호화 스위트가 적용되지 않기 때문입니다. 그 외 최소 TLS 버전에서는 TLS 1.2 이하 / TLS 1.3 암호화 스위트를 자유롭게 혼합하거나 한 종류만 지정할 수 있습니다.

> [참고] 조회 응답의 `ciphers`는 항상 TLS 1.2 이하 암호화 스위트가 먼저, TLS 1.3 암호화 스위트가 뒤에 오는 순서로 정규화되어 반환됩니다. 요청 시 보낸 원래 순서는 보존되지 않습니다.

> [주의] SSL 정책을 리스너에 연결할 경우 리스너의 TLS 버전은 해당 정책의 최소 TLS 버전과 반드시 일치해야 합니다.

> [주의] 사용자 정의 SSL 정책은 테넌트당 최대 10개까지 생성할 수 있습니다.

> [주의] SSL 정책이 하나 이상의 리스너에 연결되어 있으면 삭제할 수 없습니다. 삭제하려면 먼저 연결된 모든 리스너에서 정책을 해제해야 합니다.

> [참고] SSL 정책이 연결된 리스너는 정책의 암호화 스위트 설정이 적용됩니다. 기본 제공되는 SSL/TLS 버전별 암호화 스위트 표와 달리, 사용자가 지정한 스위트만 선택적으로 적용됩니다.
...
> [참고] 일반 로드 밸런서의 세션 연결 제한 수치의 최댓값은 60,000개, 전용 로드 밸런서의 경우 480,000개입니다. 
