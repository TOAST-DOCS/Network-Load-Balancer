...
* **TLS 1.2 이하 암호화 스위트(ciphers)**: 최소 TLS 버전이 `TLSv1.2` 이하일 때 사용할 암호화 스위트의 목록입니다.
* **TLS 1.3 암호화 스위트(tls13_ciphers)**: TLS 1.3 연결에서 사용할 암호화 스위트의 목록입니다.

> [주의] 최소 TLS 버전이 `TLSv1.3`인 경우 `ciphers`는 지정할 수 없습니다. 그 외 버전에서는 `ciphers` 또는 `tls13_ciphers` 중 최소 하나는 지정해야 합니다.
