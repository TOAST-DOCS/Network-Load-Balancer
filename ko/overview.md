...
* NHN Cloud 로드 밸런서는 TLSv1.3을 지원하기 위해 준비하고 있습니다.
...
HTTP 요청 헤더에 유효하지 않은 문자가 포함된 경우 이를 차단하는 기능입니다. 서버의 취약점을 노린 해커의 공격이나, 버그가 있는 브라우저를 통해서 유효하지 않은 문자가 포함된 HTTP 요청 헤더가 유입될 수 있습니다. 로드밸런서는 기능이 활성화 된 경우 유효하지 않은 문자가 포함된 HTTP 요청을 차단하여 인스턴스에게 전달되는것을 막으며, 400 응답 코드(Bad Request) 를 클라이언트에게 전송합니다.
