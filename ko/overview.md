...
로드밸런서는 [VPC](/Network/VPC/ko/overview/#_2)의 [서브넷](/Network/VPC/ko/overview/#_2)에 생성됩니다. 로드밸런서는 생성 시 지정된 서브넷으로부터 IP를 할당받아 자신의 IP로 사용합니다. 지정된 서브넷과 같은 VPC에 속한 인스턴스들을 멤버로 등록하여 유입된 트래픽을 분배합니다. 해당 VPC와 Peering 을 통해 연결된 다른 VPC에 속한 인스턴스들도 멤버로 추가할 수 있습니다. 그 외의 인스턴스는 멤버로 추가할 수 없습니다.
